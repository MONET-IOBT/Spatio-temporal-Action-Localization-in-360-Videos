...
class KTNConv(nn.Module):
    ...
    def forward(self, X, rows=None):
        def process_padding(pad):
            ...
            pad = torch.cat([pad[:,:,:,int(self.sphereW/2):], pad[:,:,:,:int(self.sphereW/2)]], dim=3)
            return pad
        ...
        for i, row in enumerate(rows):
            ...
            pad_height = int((kH - 1) / 2 * dilation_h)
            pad_width = int((kW - 1) / 2 * dilation_w)
...
if __name__ == "__main__":
    kernel_shapes,dilations = compute_kernelshape(imgW=40, dilation=2)
    print (kernel_shapes,len(kernel_shapes))
