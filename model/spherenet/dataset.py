...
class OmniDataset(data.Dataset):
    def __init__(self, dataset, fov=120, outshape=(60, 120),
    ...
    def __getitem__(self, idx):
        ...
        if self.aug is not None:
            ...
            dx = np.random.randint(self.outshape[1])

        img = np.array(self.dataset[idx][0], np.float32)
        if len(img.shape)==2:
            data = self.__getchan__(idx, v_c, to_flip, dx)
        else:
            img_stack = []
            for ch in range(img.shape[2]):
                img_ch = self.__getchan__(idx, v_c, to_flip, dx, ch=ch)
                img_stack.append(img_ch.unsqueeze(0))
            data = torch.cat(img_stack, dim=0)
        return data, self.dataset[idx][1]


    def __getchan__(self, idx, v_c, to_flip, dx, ch = None):
...
class OmniCIFAR10(OmniDataset):
    def __init__(self, root='datas/CIFAR10', train=True,
                 download=True, *args, **kwargs):
        self.CIFAR10 = datasets.CIFAR10(root, train=train, download=download)
        super(OmniCIFAR10, self).__init__(self.CIFAR10, *args, **kwargs)
